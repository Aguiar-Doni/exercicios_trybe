...
const mageAttak = (mage) => {
    const mana = mage.mana;
    const minDmg = mage.intelligence;
    const maxDmg = Math.floor(Math.random() * (minDmg * 2));
    const turnStart = {
        spentMana: 0,
        damageDone: 'não possui mana suficiente!'
    }
    if (mana > 15) {
        const mageDmg = maxDmg > minDmg ? maxDmg : minDmg;
        turnStart.spentMana = 15;
        turnStart.damageDone = mageDmg;
    }
    return turnStart;
}
console.log(mageAttak(mage));